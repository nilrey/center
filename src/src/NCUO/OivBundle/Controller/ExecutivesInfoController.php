...
/**
 * ExecutivesInfo controller.
 */
class ExecutivesInfoController extends Controller
{
    
    const SEC_ID = "executives_info";
    private $logger;
     ...
     *
     * @Route("/{id_oiv}/executives_info", name="executives_info_show")
     * @Method("GET")
     * @Template("ncuooiv/executives_info/show.html.twig")
     */
    public function showAction($id_oiv)
    {
        ...
        {
            $director = $em->getRepository('NCUOOivBundle:Field')->findBy(array('id_sec' => $id_oiv.'__director'), array('view_order' => 'ASC'), null , null );
            foreach( $director  as $key => $arValues) :
                if( $arValues->getIdFldContentType() == 1 ) // if type = image, then convert `&lq;` into `<`
                {
                    ...
                    $arDirector[$arValues->getIdFldShort()] = $arValues->getData();
                    if( empty($arDirector[$arValues->getIdFldShort()])){
                        $arDirector[$arValues->getIdFldShort()] = $arValues->getNoUserImgBase64();
                    }
                }
                else
                {
            ...
            $deputy_head = $em->getRepository('NCUOOivBundle:Field')->findBy(array('id_sec' => $id_oiv.'__deputy_head'), array('view_order' => 'ASC'), null , null );
            foreach( $deputy_head  as $key => $arValues) :
                if( $arValues->getIdFldContentType() == 1 ) // if type = image, then convert `&lq;` into `<`
                {
                    $tmpData = $arValues->getData();
                    $arValues->setData( htmlspecialchars_decode($tmpData ) );
                    $arDeputy[$arValues->getIdFldShort()] = $arValues->getData();
                    if( empty($arDeputy[$arValues->getIdFldShort()])){
                        $arDeputy[$arValues->getIdFldShort()] = $arValues->getNoUserImgBase64();
                    }
                }
                else
                {
        ...
        return array(
            ...
            'arDirector'   => $arDirector,
            'arDeputy'   => $arDeputy,
            'rolePerms' => $this->rolePerms,
            'urlEdit' => $this->generateUrl('executives_info_edit', array('id_oiv'=>$oiv[0]->getId() )),
            'tabmenu' => $tabMenu,
     ...
     *
     * @Route("/{id_oiv}/executives_info/edit", name="executives_info_edit")
     * @Method("GET")
     * @Template("ncuooiv/executives_info/edit.html.twig")
     */
    ...
    /**
     * Update data 
     * @Route("/{id_oiv}/executives_info/update", name="executives_info_update")
     * @Method("POST")
     * @Template("ncuooiv/executives_info/edit.html.twig")
     */
    ...
    public function prepareEditFormData($id_oiv, $data_to_html = false)
    {
        ...
        $urlAction = $this->generateUrl('executives_info_update', array('id_oiv'=>$oiv[0]->getId() ));
        $urlView = $this->generateUrl('executives_info_edit', array('id_oiv'=>$oiv[0]->getId() ));
        $urlBack = $this->generateUrl('executives_info_show', array('id_oiv'=>$oiv[0]->getId() ));
     ...
     * Finds and displays a Oiv entity.
     *
     * @Route("/{id_oiv}/director", name="director_show")
     * @Method("GET")
     * @Template("ncuooiv/oiv/show.html.twig")
     */
    public function directorAction($id_oiv)
    {
        
        return $this->redirect($this->generateUrl('executives_info_show', array('id_oiv'=> $id_oiv ) ) );
    }
    
    /**
     * Finds and displays a Oiv entity.
     *
     * @Route("/{id_oiv}/deputy_head", name="deputy_head_show")
     * @Method("GET")
     * @Template("ncuooiv/oiv/show.html.twig")
     */
    public function deputy_headAction($id_oiv)
    {
