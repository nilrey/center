...
class AdminRolesController extends Controller
{
     ...
     *
     * @Route("/admin/roles", name="oiv_admin_roles")
     * @Method("GET")
     * @Template("ncuoportal/admin/roles_list.html.twig")
     */
    public function indexAction()
    {
        ...
        $stmt = $conn->prepare('SELECT * FROM cms.roles ORDER BY id ASC');
        $stmt->execute();
     ...
     *
     * @Route("/admin/roles/new", name="oiv_admin_roles_new")
     * 
     * @Template("ncuoportal/admin/roles_new.html.twig")
     */
    public function actionNew()
    {
        
        $arResult = array();
        $arResult['urlAction'] = $this->generateUrl('oiv_admin_roles_create');
        $arResult['urlBack'] = $this->generateUrl('oiv_admin_roles');  

        $arResult['menuItem'] = array(
           "name" => "",
           "description" => "",
           "auth_type" => "AUTH_ALD_DB",
            );
     ...
     * Delete Roles Item
     *
     * @Route("/admin/roles/{id_roles}/delete", name="oiv_admin_roles_delete")
     * 
     * @Template("ncuoportal/admin/roles_list.html.twig")
     */
    public function actionDelete($id_roles)
    {
        ...
        if ( in_array(intval($id_roles), array(1, 2, 3, 5, 6) ) ){
            $message = array(
                'message_text'  => 'Удаление основной роли невозможно.',
                'message_error' => true,
                'error_explain' => ''
            );
        }elseif ( $this->isUserGranted() ){

            $sql_tmpl = 'DELETE FROM cms.roles WHERE id=:id_roles';
            $stmt = $conn->prepare($sql_tmpl);
            $stmt->bindValue(':id_roles', intval($id_roles) );
            $res = $stmt->execute();
     ...
     * Save new item roles
     *
     * @Route("/admin/roles/create", name="oiv_admin_roles_create")
     * @Method("POST")
     * @Template("ncuoportal/admin/roles_new.html.twig")
     */
    public function actionCreateNew(Request $request)
    {
        ...
        if ( is_array($request->request->get("FIELDS") ) )
        {
            ...
            elseif( empty($arData['description']) )
            {
                $message = array(
                    'message_error' => true,
                    'message_text'  => "Поле `Описание` должно быть заполнено.",
                    'error_explain' => ''
                ); 
            }
            else
            {
                if( $arData['auth_type'] == 'AUTH_ALD_DB' ) $arData['auth_type'] = $arData['auth_priority'];
                $stmt = $conn->prepare('SELECT max(id) FROM cms.roles');
                $stmt->execute();        
                $id_max = $stmt->fetchAll();
                $id_insert = $id_max[0]['max']+1;
                $sql_tmpl = 'INSERT INTO cms.roles (id, name, description, auth_type  ) VALUES (:id , :name, :description, :auth_type )';
                ...
                $stmt->bindValue(':description', trim($arData['description']) );
                $stmt->bindValue(':auth_type', trim($arData['auth_type']) );
                $res = $stmt->execute();
        ...
        $arResult['urlAction'] = $this->generateUrl('oiv_admin_roles_create', array() );
        $arResult['urlBack'] = $this->generateUrl('oiv_admin_roles');
     ...
     *
     * @Route("/admin/roles/{id_roles}/edit", name="oiv_admin_roles_edit")
     * @Method("GET")
     * @Template("ncuoportal/admin/roles_edit.html.twig")
     */
    public function editAction($id_roles)
    {

        $arResult = $this->prepareEditFormData($id_roles);
     ...
     *  
     * @Route("/admin/roles/{id_roles}/update", name="oiv_admin_roles_update")
     * @Method("POST")
     * @Template("ncuoportal/admin/roles_edit.html.twig")
     */
    
    public function updateAction(Request $request, $id_roles){
        $arData = array();
        $message = array(
            'message_text'  => '',
            'message_error' => false,
        ...
        if ( is_array($request->request->get("FIELDS") ) )
        {
            ...
            else
            {
                if( $arData['auth_type'] == 'AUTH_ALD_DB' ) $arData['auth_type'] = $arData['auth_priority'];
                try{
                    $sql_tmpl = 'UPDATE cms.roles SET';
                    if ( intval($id_roles) > 6 && !empty($arData['name'])) $sql_tmpl .= ' name=:name, ';
                    $sql_tmpl .= ' description=:description, auth_type=:auth_type WHERE id=:id_role';
                    $stmt = $this->container->get('doctrine.orm.entity_manager')->getConnection()->prepare($sql_tmpl);
                    if ( intval($id_roles) > 6 && !empty($arData['name'])) $stmt->bindValue(':name', trim($arData['name']));
                    $stmt->bindValue(':description', trim($arData['description']));
                    $stmt->bindValue(':id_role', intval($id_roles) );
                    $stmt->bindValue(':auth_type', trim($arData['auth_type']) );
                    $res = $stmt->execute();
    ...
    public function prepareEditFormData($id_roles, $data_to_html = false)
    {
        $conn = $this->getDoctrine()->getManager()->getConnection();

        $stmt = $conn->prepare('SELECT * FROM cms.roles WHERE id=:id_roles');

        $stmt->execute(["id_roles"=>$id_roles]);        
        $arItem = $stmt->fetchAll();

        // динамически формируем URL для  выполнения операции обновления пользователя в БД
        $urlAction = $this->generateUrl('oiv_admin_roles_update', array('id_roles'=>$arItem[0]["id"] ));
        $urlView = $this->generateUrl('oiv_admin_roles');
        $urlBack = $this->generateUrl('oiv_admin_roles');
